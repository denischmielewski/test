...
RcfProtoController::RcfProtoController() : mpRcfChannel(NULL), mpRcfSession(NULL)
{
}

RcfProtoController::RcfProtoController(RcfProtoSession & session) :
    mpRcfChannel(NULL),
    mpRcfSession(&session)
{
}
...
{
    mpRcfSession->SetFailed(reason);
}

bool RcfProtoController::IsCanceled() const
{
    return mpRcfSession->IsCanceled();
}

void RcfProtoController::NotifyOnCancel(Closure* callback)
{
    mpRcfSession->NotifyOnCancel(callback);
}
...
RcfSession& RcfProtoSession::getRcfSession()                    { return mRcfSession; }
    ...
    RCF::ByteBuffer         requestBuffer)
{
    ...
    RcfProtoSessionPtr rcfProtoSessionPtr( new RcfProtoSession(rcfSession) );

    if (mpSwigCallback)
    {
        _SwigCallbackArgs args;
        _SwigCallbackArgs * pArgs = &args;
        mpSwigCallback->ProtoRpcBegin(pArgs, this, rcfProtoSessionPtr.get(), serviceName, methodId);
        if (pArgs->mErrorString.size() > 0)
        {
            rcfProtoSessionPtr->SetFailed( pArgs->mErrorString );
            rcfProtoSessionPtr->_Commit();
        }
    }
    else
    {
        ProtoRpcBeginCpp(*rcfProtoSessionPtr, serviceName, methodId);
    }
